#!/bin/bash
# ─── SureStrikeTips — One-command startup ──────────────────────────────────────────
...
# ── 1. Kill anything already running on SureStrikeTips ports ─────────────────────
echo "⏹  Stopping old SureStrikeTips processes..."
lsof -ti:5003 | xargs kill -9 2>/dev/null || true
lsof -ti:3002 | xargs kill -9 2>/dev/null || true
# Kill only SureStrikeTips cloudflare tunnels (identified by port)
...
cloudflared tunnel --url http://localhost:5003 > /tmp/bt-tunnel-backend.log 2>&1 &
sleep 8
...
cd "$FRONTEND_DIR"
PORT=3002 npm run dev > /tmp/bt-frontend.log 2>&1 &
FRONTEND_PID=$!
...
cloudflared tunnel --url http://localhost:3002 > /tmp/bt-tunnel-frontend.log 2>&1 &
sleep 8
